...
<!-- END SIGMA IMPORTS -->
<script type="text/javascript" src="http://10.30.105.240:8080/socket.io/socket.io.js"></script>
<script type="text/javascript">
  socket = io.connect('http://10.30.105.240:8080');
  socket.on('clic', function(data) {
    hizoClic(data.x, data.y);
  });
</script>
...
(function() {
  ...
  dom.addEventListener('click', function(e) {
    var x = sigma.utils.getX(e) - dom.offsetWidth / 2;
    var y = sigma.utils.getY(e) - dom.offsetHeight / 2;
    console.log('enviando hiceClic a server', {x:x, y:y});
    socket.emit('hiceClic', {x:x, y:y});
  }, false);

  window.hizoClic = function(x,y) {
    // Find neighbors:
    var x,
        y,
        p,
